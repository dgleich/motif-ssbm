...
# We got all the edge stuff right. And we got conductance for the
# triangle case right (after Austin's correction), but we still aren't
# finding the reason for the gap.
# which is because we don't have a good idea of the spectrum of the
# weighted matrices, and taking an ER-random cases don't seem to do a good job.
# On reflection, these seem like Marchenko-pastur laws, so I'm going to investigate
# now 
...
lbs2 = map(
  μ -> begin
    ...
    pbar = mean(W)
    @show pbar
    return 1-(3/(m*k*pbar))^(1/2)
  end, μs)

lbs_sq = map( μ -> begin
                    q = μ*p/((1-μ)*(k-1))
                    pbar = (q*(k)*(k-1)+p*k)/(k^2)
                    return 1-(3/(m*k*pbar))^(1/3)
                  end,
             μs )
  plot!(lbs, collect(μs))
