...
## Conditioning measures and a sweep
...
function conditioning_measures(mat)
  ...
  vals[:CombinatorialLaplacian] = cond(full(mats[:CombinatorialLaplacian]))
  vals[:WeightedAdjacency] = cond(full(mats[:WeightedAdjacency]))
  vals[:Adjacency] = cond(full(mats[:WeightedAdjacency]))

  f,lam2 = fiedler_vector(mat)
  if abs(lam2) >= 100*eps(1.0)
    z = dhalf.^2.*f
    vals[:Fiedler] = norm(f)*norm(z)/abs(f'*z)
  else
    vals[:Fiedler] = 1.0
  end
